...
// Confetti pieces for win screen
...
export default function Game({ gameState }: GameProps) {
    ...
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/75 backdrop-blur-lg">
        ...
        <div className="glass rounded-3xl p-8 max-w-sm w-full mx-4 text-center shadow-2xl space-y-4 animate-winPop">
          ...
          <h2 className={`text-4xl font-black ${iWon ? 'text-yellow-400 [text-shadow:0_0_30px_#eab30880]' : 'text-white'}`}>
          ...
          {gameState.isHost ? (
            <button
              ...
              className="w-full py-3.5 rounded-2xl font-black text-lg text-white transition-all active:scale-95
                         ...
                         shadow-xl shadow-green-500/30 hover:shadow-green-500/50 hover:scale-[1.02]"
            >
              ▶ Play Again
            </button>
          ) : (
            <p className="text-white/35 text-sm animate-pulse2">Waiting for host to restart…</p>
          )}
        </div>
      </div>
    );
  ...
  return (
    <div className="flex flex-col h-screen select-none overflow-hidden">
      ...
      {/* ── Top bar ──────────────────────────────────────────────────────────── */}
      <div className="flex items-center justify-between px-3 py-2 glass border-b border-white/5 shrink-0">
        {/* Logo */}
        ...
        {/* Center: turn + direction */}
        <div className="flex items-center gap-2">
          {isMyTurn && !gameState.paused ? (
            <span className="text-green-400 text-xs font-black animate-pulse2
                             bg-green-400/10 border border-green-400/25 px-2.5 py-0.5 rounded-full">
              YOUR TURN
            </span>
          ) : (
          ...
          <span className="text-white/30 text-base font-bold animate-float" style={{ animationDuration: '2s' }}>
            {dirArrow}
          </span>
        </div>

        {/* Right: theme + room code */}
        <div className="flex items-center gap-2">
          <button
            onClick={toggleTheme}
      ...
      {gameState.paused && pausedPlayer && (
        <div className="bg-yellow-500/20 border-b border-yellow-500/25 px-3 py-1.5
                        text-center text-yellow-300 text-xs font-semibold animate-pulse2 shrink-0">
          ⚡ {pausedPlayer.name} disconnected — reconnecting in 30s…
        </div>
      )}

      {/* ── Opponents ────────────────────────────────────────────────────────── */}
      <div className="flex gap-2 px-3 py-2 overflow-x-auto shrink-0 justify-center scrollbar-none">
        {opponents.length === 0 ? (
          <p className="text-white/25 text-xs py-3">No opponents yet</p>
        ) : opponents.map(p => (
          <OpponentPanel
            key={p.id}
            player={p}
      ...
      <div className="flex-1 flex flex-col items-center justify-center gap-3 px-3 min-h-0">
        ...
        <div ref={logRef} className="w-full max-w-xs h-14 overflow-y-auto px-2 space-y-0.5">
          {gameState.log.slice(-5).map((entry, i) => (
            <p key={i} className="text-white/35 dark:text-white/35 text-[11px] text-center leading-snug truncate">
              {entry}
            </p>
          ))}
        </div>
      ...
      {/* ── Error toast ───────────────────────────────────────────────────────── */}
      {error && (
        <div className="mx-3 mb-1 bg-red-500/15 border border-red-500/30 rounded-2xl
                        px-3 py-1.5 text-red-300 text-xs text-center shrink-0 animate-fadeIn">
          ⚠ {error}
        </div>
      )}
      ...
      <div className="flex items-center justify-center gap-3 px-3 pb-4 shrink-0">
        {isMyTurn && gameState.drawnCardId !== null && (
          <button
            ...
            className="px-5 py-2.5 rounded-2xl font-bold glass text-white dark:text-white
                       hover:bg-white/10 transition-all active:scale-90 border border-white/10"
          >
            Pass
          </button>
        )}
        ...
        {showUno && (
          <button
            ...
            className="
              px-7 py-3 rounded-2xl font-black text-xl text-white
