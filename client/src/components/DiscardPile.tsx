...
export default function DiscardPile({
  discardTop, currentColor, onDrawClick, drawPending, canDraw,
}: DiscardPileProps) {
  const prevId      = useRef(-1);
  const [landing, setLanding]       = useState(false);
  const [deckShaking, setDeckShaking] = useState(false);

  // Trigger land animation when discard top changes
  useEffect(() => {
  ...
  return (
    <div className="flex items-center gap-8 justify-center">
      ...
      <div className="flex flex-col items-center gap-2">
        ...
        >
          ...
          {[2, 1, 0].map(offset => (
            <div
              key={offset}
              className="absolute"
              style={{ top: -offset * 2, left: offset * 1, zIndex: offset === 0 ? 1 : 0 }}
            >
              <Card faceDown />
            </div>
          ))}
        ...
        <span className={`text-xs font-bold ${drawPending > 0 ? 'text-red-400' : 'text-white/40 dark:text-white/40'}`}>
      ...
      <div className="flex flex-col items-center gap-2">
        <div
          className={`
            ...
            ${discardTop.color === 'wild' ? `ring-4 ring-offset-2 ring-offset-transparent ${ring}` : ''}
          `}
          style={discardTop.color === 'wild' ? { '--tw-ring-offset-color': 'transparent' } as React.CSSProperties : {}}
        >
          <Card card={discardTop} landing={landing} />
        </div>

        {/* Active colour indicator for wilds */}
        ...
        {discardTop.color !== 'wild' && (
          <span className="text-xs text-white/40 dark:text-white/40">Discard</span>
        )}
      </div>
    </div>
  );
