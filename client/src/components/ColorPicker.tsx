  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/60 backdrop-blur-md">
      <div className="glass rounded-t-3xl sm:rounded-3xl p-6 w-full max-w-sm mx-0 sm:mx-4 animate-slideUp shadow-2xl">
        <div className="w-10 h-1 bg-black/15 dark:bg-white/20 rounded-full mx-auto mb-5 sm:hidden" />
        <h2 className="text-gray-900 dark:text-white font-black text-xl text-center mb-1">Choose a Color</h2>
        <p className="text-gray-500 dark:text-white/40 text-xs text-center mb-5">The wild card will match your choice</p>
        <div className="grid grid-cols-2 gap-3">
          {COLORS.map(c => (
            <button key={c.value} onClick={() => onSelect(c.value)}
              className={`bg-gradient-to-br ${c.from} ${c.to}
                rounded-2xl py-6 text-white font-black text-lg
                transition-all active:scale-90 hover:scale-105 shadow-xl ${c.shadow}
                flex flex-col items-center justify-center gap-2 border border-white/20`}>
        ...
        <button onClick={onCancel}
          className="mt-4 w-full py-2.5 text-gray-500 dark:text-white/40 hover:text-gray-700 dark:hover:text-white/65
                     text-sm transition-colors rounded-xl hover:bg-black/5 dark:hover:bg-white/5">
          Cancel
        </button>
      </div>
    </div>
