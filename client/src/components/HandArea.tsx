...
export default function HandArea({
  ...
  function handleCardClick(card: UnoCard) {
    ...
    if (card.value === 'wild' || card.value === 'wild4') onNeedColor(card.id);
    else onPlayCard(card.id);
  }

  const count     = hand.length;
  const maxSpread = Math.min(44, count > 1 ? Math.min(44, (window.innerWidth * 0.82) / count) : 0);
  const maxRot    = Math.min(3, 18 / count);

  return (
    <div className="w-full flex flex-col items-center gap-1">
      <div className="relative flex items-end justify-center" style={{ height: '7.5rem', width: '100%' }}>
        {hand.map((card, i) => {
          ...
          return (
            <div key={card.id} className="absolute bottom-0 card-transition"
              style={{ transform: `translateX(${offset}px) translateY(${arc}px) rotate(${rot}deg)`, zIndex: i, transformOrigin: 'bottom center' }}>
              <Card card={card} playable={playable} highlighted={isHighlighted} drawing={drawing}
